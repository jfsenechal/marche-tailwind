<script setup>
//https://www.getrevue.co/profile/shrutibalasa/issues/where-is-inline-flex-used-1394402
</script>
<template>
    <ul class="flex xl:row-start-2 xl:row-end-2 xl:col-start-3 xl:col-end-4">
      <li
        class="mx-2 flex items-center border border-cta-dark px-2 py-2 text-cta-dark transition-all duration-500 ease-in-out hover:bg-cta-dark hover:text-white"
      >
        <a href="https://www.marche.be/./actualites/">Actualités</a>
      </li>
      <li
        class="m-auto mx-2 flex border border-cta-dark px-2 py-2 text-cta-dark transition-all duration-500 ease-in-out hover:bg-cta-dark hover:text-white"
      >
        <a href="https://www.marche.be/./infos-travaux/">Infos Travaux</a>
      </li>
    </ul>
</template>
