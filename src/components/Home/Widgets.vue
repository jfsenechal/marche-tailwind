...
<template>
  <div class="grid grid-cols-1 xl:grid-cols-[repeat(5,1fr)] gap-2 pt-6" id="widgets">
    ...
    >
      <i
        :class="'absolute inset-0 block h-32 bg-cover bg-center bg-no-repeat grayscale ' + item.class"
      ></i>
      <a
        :href="item.url"
        class="pt-4 w-full bg-widget-color hover:bg-widget-color-hover group relative flex h-32 flex-col items-center justify-start text-white transition-all duration-700"
      >
        <h3
          class="text-xl font-montserrat-semi-bold relative top-6 transition-all duration-700 group-hover:top-0 group-hover:ease-in-out"
        >
          {{ item.title }}
        </h3>
        <span
          class="text-lg relative -bottom-20 pt-4 text-white transition-all delay-300 duration-700 group-hover:bottom-0"
        >
          {{ item.intro }}
        </span>
      </a>
